...
import { AntlersNode, AntlersParserFailNode, ConditionNode, EscapedContentNode, ExecutionBranch, RecursiveNode } from '../nodes/abstractNode.js';
...
export class TagPairAnalyzer {
    constructor() {
        this.closingTagIndex = new Map();
        this.closingTagIndexCount = new Map();
        this.openTagIndexCount = new Map();
        this.closingTagNames = new Map();
                    ...
                    if (node.endPosition.isBefore(candidateNode.startPosition)) {
                        ...
                        if (candidateNode.isClosingTag == false) {
                            if (scanFor.includes(compound)) {
                                const refOpen = this.openTagIndexCount.get(this.stackCount)?.get(compound), refClose = this.closingTagIndexCount.get(this.stackCount)?.get(compound);
                                if (refOpen != refClose) {
            ...
            nodes.forEach((node) => {
                if ((node instanceof AntlersNode || node instanceof AntlersParserFailNode) && node.isClosedBy != null) {
                    if (this.document != null) {
                        const content = this.document.getText((node.endPosition?.index ?? 0) + 1, (node.isClosedBy.startPosition?.index ?? 0));
                        node.runtimeContent = content;
                    }
                }
            });
            for (let i = 0; i < nestedNodes.length; i++) {
                const node = nestedNodes[i], refId = node.refId ?? '';
                ...
                if ((node instanceof AntlersNode || node instanceof AntlersParserFailNode) &&
                    ...
                    if (this.document != null) {
                        content = this.document.getText((node.endPosition?.index ?? 0) + 1, (node.isClosedBy?.startPosition?.index ?? 0));
                    }
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if ((node instanceof AntlersNode || node instanceof AntlersParserFailNode) && node.isComment == false && node.isClosingTag == false && node.isClosedBy != null) {
                ...
                if (compound == 'if') {
                    ...
                    const conditionStart = (node.startPosition?.offset ?? 0), finalClosing = node.getFinalClosingTag(), conditionEnd = (finalClosing.endPosition?.offset ?? 0) + 1;
                    ...
                    while (doContinue) {
                        ...
                        if ((node instanceof AntlersNode || node instanceof AntlersParserFailNode)) {
                            ...
                            const branchStart = (node.endPosition?.offset ?? 0) + 1, branchEnd = (node.isClosedBy?.startPosition?.offset) ?? 0, documentText = document.getText(branchStart, branchEnd);
...
TagPairAnalyzer.ForceBreakLimit = 100000;
//# sourceMappingURL=tagPairAnalyzer.js.map