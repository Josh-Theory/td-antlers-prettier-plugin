...
import { AntlersNode, AntlersParserFailNode, CommentParserFailNode, ConditionNode, EscapedContentNode, LiteralNode, PhpExecutionNode, PhpParserFailNode, VariableNode } from '../nodes/abstractNode.js';
import { Position } from '../nodes/position.js';
import { GlobalRuntimeState } from '../runtime/globalRuntimeState.js';
import { StringUtilities } from '../utilities/stringUtilities.js';
import { Md5 } from 'ts-md5/dist/md5.js';
import { TagIdentifier } from '../nodes/tagIdentifier.js';
import { AntlersNodeParser } from './antlersNodeParser.js';
import { TagPairAnalyzer } from '../analyzers/tagPairAnalyzer.js';
import { RecursiveParentAnalyzer } from '../analyzers/recursiveParentAnalyzer.js';
import { LanguageParser } from './languageParser.js';
import { WordScanner } from '../document/scanners/wordScanner.js';
import { VirtualHierarchy } from './virtualDocument/virtualHierarchy.js';
import TagManager from '../../antlers/tagManagerInstance.js';
import { ModifierAnalyzer } from '../analyzers/modifierAnalyzer.js';
...
import { InlineNodeAnalyzer } from '../analyzers/inlineNodeAnalyzer.js';
export class DocumentParser {
    constructor() {
        this.interpolationRegions = new Map();
        ...
        this.interpolationEndOffsets = new Map();
        ...
        this.antlersStartPositionIndex = new Map();
        this.chunkSize = 5;
        this.maxLine = 1;
    ...
    parse(text) {
            ...
            for (let i = 0; i < indexCount; i += 1) {
                ...
                if (i == 0 && offset > 0 && !this.isNoParse) {
                    ...
                    if (node.content.length > 0) {
                        ...
                        const startOffset = (node.startPosition.index ?? 0), endOffset = (node.endPosition.index) + 1;
                            ...
                            if (nextAntlersStart < this.lastAntlersNode.endPosition.offset) {
                                ...
                                if (skipIndex == null) {
                                    ...
                                    if (nodeContent.length > 0 && !this.isNoParse) {
                                        ...
                                        const startOffset = (literalNode.startPosition.index ?? 0), endOffset = (literalNode.endPosition.index) + 1;
            ...
            if (chain.modifierChain.length > 0) {
                chain.modifierChain.forEach((modifier) => {
                    if (modifier.nameNode != null && ModifierManager.instance != null && ModifierManager.instance.hasModifier(modifier.nameNode?.name)) {
                        modifier.modifier = ModifierManager.instance?.getModifier(modifier.nameNode.name);
                    }
                });
            }
        });
        ...
        this.nodes.forEach((node) => {
            if (node instanceof AntlersNode) {
                ...
                if (node.isTagNode) {
                    const tagReference = TagManager.instance?.findTag(node.runtimeName());
                    if (tagReference != null) {
                        node.requiresClose = tagReference.requiresCloseResolver != null
                            ? tagReference.requiresCloseResolver(node)
                            : tagReference.requiresClose;
                        node.allowsContentClose = tagReference.allowsContentClose;
                    }
                }
        ...
        this.nodes.forEach((node) => {
            if (node instanceof AntlersNode) {
                if (node.isClosedBy != null) {
                    const startOffset = (node.endPosition?.offset ?? 0) + 1, endOffset = node.isClosedBy.startPosition?.offset ?? 0, nodeStartOffset = (node.startPosition?.offset ?? 0), nodeEndOffset = (node.isClosedBy.endPosition?.offset ?? 0) + 1;
    ...
    createChildDocuments(renderNodes) {
        renderNodes.forEach((node) => {
            if (node instanceof AntlersNode && node.isClosedBy != null) {
                const isClosedBy = node.isClosedBy, docStart = (node.startPosition?.index ?? 0) - 1, docLength = (isClosedBy.endPosition?.index ?? 0) - docStart;
                node.documentText = this.content.substr(docStart, docLength);
                if (this.parseChildDocuments) {
                    const startOffset = (node.endPosition?.index ?? 0), length = ((isClosedBy.startPosition?.index ?? 0) - 1) - startOffset, childText = this.content.substr(startOffset, length);
    ...
    parseIntermediateText() {
        ...
        for (this.currentIndex = 0; this.currentIndex < this.inputLen; this.currentIndex += 1) {
            ...
            if (this.cur == DocumentParser.AtChar && this.next != null && this.next == DocumentParser.LeftBrace) {
                ...
                this.currentContent = this.currentContent.concat(DocumentParser.LeftBrace.repeat(leftBraceCount).split(''));
                this.sourceContent = this.currentContent.concat(DocumentParser.LeftBrace.repeat(leftBraceCount).split(''));
    ...
    processInputText(input) {
        ...
        for (let i = 0; i < newLineCountLen; i++) {
            const thisNewLine = documentNewLines[i], thisIndex = thisNewLine.index ?? 0;
    ...
    scanToEndOfPhpRegion(checkChar) {
        ...
        for (this.currentIndex; this.currentIndex < this.inputLen; this.currentIndex += 1) {
            ...
            if (this.next == null) {
                ...
                this.doesHaveUnclosedStructures = true;
                failNode.pushError(AntlersError.makeSyntaxError(AntlersErrorCodes.TYPE_INCOMPLETE_PHP_EVALUATION_REGION, failNode, 'Unexpected end of input while parsing Antlers PHP region.'));
                break;
            }
        }
    }
    ...
    scanToEndOfAntlersCommentRegion() {
        ...
        for (this.currentIndex; this.currentIndex < this.inputLen; this.currentIndex += 1) {
            ...
            if (this.next == null) {
                ...
                this.lastAntlersNode = failNode;
                this.doesHaveUnclosedStructures = true;
                failNode.pushError(AntlersError.makeSyntaxError(AntlersErrorCodes.TYPE_INCOMPLETE_ANTELRS_COMMENT_REGION, failNode, 'Unexpected end of input while parsing Antlers comment region.'));
                break;
            }
        }
    }
    scanToEndOfInterpolatedRegion() {
        ...
        const content = subContent.join(''), varSlug = 'int_' + Md5.hashStr(content);
    ...
    scanToEndOfAntlersRegion() {
        ...
        for (this.currentIndex; this.currentIndex < this.inputLen; this.currentIndex += 1) {
            ...
            if (this.next == null) {
                ...
                failNode.pushError(AntlersError.makeSyntaxError(AntlersErrorCodes.TYPE_INCOMPLETE_ANTLERS_REGION, failNode, message));
                break;
            }
        }
    }
    makeAntlersPhpNode(index, isEcho) {
        ...
        if (index + 3 > this.inputLen) {
            ...
            this.doesHaveUnclosedStructures = true;
            node.pushError(AntlersError.makeSyntaxError(AntlersErrorCodes.TYPE_UNEXPECTED_EOI_WHILE_MANIFESTING_ANTLERS_NODE, node, 'Unexpected end of input while locating end of Antlers region.'));
            return node;
        }
