...
import { AbstractNode, ParameterNode, RecursiveNode } from '../nodes/abstractNode.js';
...
export class AntlersNodeParser {
    ...
    parseNode(node) {
        ...
        if (node.content.startsWith('*subrecursive')) {
            ...
            const pathStartPosition = node.relativeOffset(node.content.indexOf(nodeContent), node.content.indexOf(nodeContent));
            if (!nodeContent.trim().startsWith('[')) {
                recursiveNode.pathReference = this.pathParser.parse(nodeContent, pathStartPosition);
            }
        ...
        for (this.currentIndex; this.currentIndex < this.inputLen; this.currentIndex += 1) {
            ...
            if (hasFoundName == false) {
                if (isParsingString == false && (this.cur == DocumentParser.String_Terminator_DoubleQuote ||
                ...
                // Terminate the name at `(` so function-call-style tags like
                // `{{ switch(x => 'one') }}` expose `switch` as the node name.
                // Without this, the name is parsed as `switch(x`, `isTagNode`
                // becomes false, and linters may incorrectly treat the node as
                // a variable and apply a `$` disambiguation fix.
                if (this.cur == DocumentParser.LeftParen && nameContent.length > 0) {
                    name = nameContent.join('');
                    hasFoundName = true;
                    break;
                }
        ...
        if (name.startsWith('[') == false) {
            node.pathReference = this.pathParser.parse(name, node.nameStartsOn);
        }
        ...
        if (node.name.name == 'unless') {
            ...
            replacedNode.name.compound = 'if';
            if (node.isClosingTag == false) {
                const originalContent = node.getContent() ?? '', unlessContent = ' if !(' + originalContent + ') ';
                ...
                this.remapSyntheticRuntimePositions(replacedNode, node, originalContent);
            ...
            const originalContent = node.getContent() ?? '', unlessContent = ' elseif !(' + originalContent + ') ';
            replacedNode.content = unlessContent;
            replacedNode.originalNode = node;
            replacedNode.resetContentCache();
            replacedNode.runtimeNodes = this.lexer.tokenize(replacedNode, replacedNode.getContent() ?? '');
    ...
    // The unless/elseunless rewrite replaces `{{ unless X }}` with a synthetic
    // `{{ if !(X) }}` and re-lexes that string. The lexer's positions are
    // therefore offsets into the synthetic content, not the original source —
    // which breaks consumers that use parser-reported positions for source
    // mapping (semantic-tokens highlighting, refactoring, etc.).
    //
    // This walks the freshly-lexed runtime nodes and remaps each position back
    // to the original source. Tokens that fall in the synthetic prefix (` !(`)
    // or suffix (`) `) are marked virtual so consumers can filter them.
    remapSyntheticRuntimePositions(replacedNode, originalNode, originalContent) {
        const parser = replacedNode.getParser();
        if (parser == null) {
            return;
        }
        if (replacedNode.runtimeNodes == null || replacedNode.runtimeNodes.length == 0) {
            return;
        }
        // Both rewrites produce a synthetic-getContent of
        //     ` !(<originalContent>) `
        // because the inner stripping of the keyword (`if` / `elseif`) leaves
        // a leading space. The prefix is therefore always 3 characters and
        // the suffix is 2.
        const prefixLen = 3;
        const originalLen = originalContent.length;
        const replacedRawStart = replacedNode.rawStart.length;
        const replacedStartIndex = replacedNode.startPosition?.index ?? 0;
        // Where in the source does `originalContent` (i.e. originalNode.getContent())
        // begin? originalNode.content holds the full inner text with the keyword
        // intact; getContent strips the keyword, so the difference in length
        // tells us how far in the keyword + leading whitespace runs.
        const keywordSpan = originalNode.content.length - originalLen;
        const originalContentSourceStart = (originalNode.startPosition?.offset ?? 0) +
            originalNode.rawStart.length +
            keywordSpan;
        const originalContentSourceIndex = (originalNode.startPosition?.index ?? 0) +
            originalNode.rawStart.length +
            keywordSpan;
        const collapseTo = originalNode.startPosition;
        for (const runtimeNode of replacedNode.runtimeNodes) {
            const startPos = runtimeNode.startPosition;
            const endPos = runtimeNode.endPosition;
            if (startPos == null || endPos == null) {
                continue;
            }
            // Recover the synthetic-content offset the lexer used. Lexer
            // positions are computed via `replacedNode._lexerRelativeOffset`,
            // which on the rewritten node (no `nameStartsOn`) reduces to
            // `syntheticOffset + rawStart.length + startPosition.index`.
            const startSynthetic = startPos.offset - replacedRawStart - replacedStartIndex;
            const endSynthetic = endPos.offset - replacedRawStart - replacedStartIndex;
            const startInOriginal = startSynthetic >= prefixLen && startSynthetic <= prefixLen + originalLen;
            const endInOriginal = endSynthetic >= prefixLen && endSynthetic <= prefixLen + originalLen;
            if (startInOriginal && endInOriginal) {
                const newStartOffset = originalContentSourceStart + (startSynthetic - prefixLen);
                const newEndOffset = originalContentSourceStart + (endSynthetic - prefixLen);
                const newStartIndex = originalContentSourceIndex + (startSynthetic - prefixLen);
                const newEndIndex = originalContentSourceIndex + (endSynthetic - prefixLen);
                runtimeNode.startPosition = parser.positionFromOffset(newStartOffset, newStartIndex, true);
                runtimeNode.endPosition = parser.positionFromOffset(newEndOffset, newEndIndex, true);
            }
            else {
                runtimeNode.isVirtual = true;
                if (collapseTo != null) {
                    runtimeNode.startPosition = collapseTo;
                    runtimeNode.endPosition = collapseTo;
                }
            }
        }
    }
    ...
    getParameters(node) {
        const content = node.getContent() ?? '', chars = content.split(''), improperStringCandidates = [], parameters = [], charCount = chars.length;
        ...
        for (let i = 0; i < charCount; i++) {
            ...
            if (hasFoundName && ((terminator != null && current == terminator) ||
...
//# sourceMappingURL=antlersNodeParser.js.map