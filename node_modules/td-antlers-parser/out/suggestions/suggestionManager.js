...
import { StringValueNode, EqualCompOperator, VariableNode, PhpExecutionNode } from '../runtime/nodes/abstractNode.js';
...
const ConditionalCompletionTriggers = ["if", "elseif", "unless", "elseunless"];
const InvalidTriggers = ["else", "/if"];
function isCursorInsideSymbol(symbol, position) {
    const startLine = symbol.startPosition?.line ?? -1, endLine = symbol.endPosition?.line ?? - -1, startOffset = symbol.startPosition?.char ?? -1, endOffset = symbol.endPosition?.char ?? -1, checkLine = position.line + 1, checkChar = position.character + 1;
    if (startLine == endLine) {
        if (checkLine == startLine + 1 && (checkChar > startOffset &&
            checkChar <= endOffset)) {
            return true;
        }
    }
...
export function getCurrentSymbolMethodNameValue(params) {
    let valueToReturnn = "";
...
export function getAbsoluteRoot(word) {
    if (word.includes(":") == false) {
        return word;
    }
    return word.split(":")[0];
}
export function convertImmediateScopeToCompletionList(params) {
...
function mergeCustomModifiers(items) {
    ProjectManager.instance?.getStructure().getCustomModifierNames().forEach((name) => {
        items.push({
...
function getModifierCompletionList() {
    const items = [];
...
export class SuggestionManager {
    static getSuggestions(params) {
        ...
        let completionItems = [], injectParentScope = true;
        ...
        if (params.currentNode?.isEmpty() ||
            ...
            if (params.context?.modifierContext == null) {
                const allTagNames = TagManager.instance?.getVisibleTagsWithDocumentation() ?? [], addedTagNames = [], tagCompletions = [];
                for (let i = 0; i < allTagNames.length; i++) {
                    ...
                    if (allTagNames[i].label.includes(":") == false) {
                        ...
                        const adjustedTagName = allTagNames[i].label.split(":")[0];
        ...
        if (lastScopeItem != null && lastScopeItem.currentScope != null) {
                ...
                if (params.context.feature != null &&
                    ...
                    if (specialVarNames.includes(params.context.feature.prev.prev.name)) {
                        ...
                        if (currentDataType == 'replicator' || currentDataType == 'bard') {
                            const field = params.context.feature.prev.prev.scopeVariable.sourceField;
            ...
            if (lastScopeItem.currentScope.hasListInHistory(lastScopeItem.runtimeName()) == false &&
                lastScopeItem.currentScope.hasPristineReference(lastScopeItem.runtimeName()) == false) {
            ...
            if (lastScopeItem.runtimeName().includes(":")) {
                if (lastScopeItem.currentScope.containsPath(trimRight(lastScopeItem.runtimeName(), ":"))) {
                    const checkPath = trimRight(lastScopeItem.runtimeName(), ":"), checkWord = trimLeft(trimRight(params.leftWord, ":"), "{");
                    ...
                    if (checkPath == checkWord) {
                        ...
                        if (specificScope != null) {
                            specificScope.lists.forEach((val, key) => {
                                completionItems.push(makeFieldSuggest(key, "", ""));
                            });
                            specificScope.values.forEach((val) => {
                                if (val.sourceField != null) {
                                    completionItems.push({
                                        label: val.name,
                                        detail: val.sourceField.blueprintName,
            ...
            if (lastScopeItem.isTagNode) {
                if (params.isInVariableInterpolation &&
                    ...
                    if (params.context?.parameterContext != null && params.context.parameterContext.parameter != null) {
                        ...
                        if (tActiveParam.isVariableReference &&
                            ...
                            if (params.position.character <= (tActiveParam.valuePosition?.end?.char ?? -1)) {
                                let tagParameter = null, didSearchForDynamicParameter = false;
                                if (TagManager.instance?.canResolveDynamicParameter(lastScopeItem.runtimeName())) {
                                    const tagRef = TagManager.instance.findTag(lastScopeItem.runtimeName());
                                    if (typeof tagRef !== "undefined" &&
                                        typeof tagRef.resolveDynamicParameter !== "undefined" &&
                                        tagRef.resolveDynamicParameter != null) {
                                        tagParameter = tagRef.resolveDynamicParameter(lastScopeItem, tActiveParam.name);
                                        didSearchForDynamicParameter = true;
                                    }
                                }
                                else {
                                    tagParameter = TagManager.instance?.getParameter(lastScopeItem.runtimeName(), tActiveParam.name) ?? null;
                                }
                                if (tagParameter == null && didSearchForDynamicParameter) {
                                    tagParameter = TagManager.instance?.getParameter(lastScopeItem.runtimeName(), tActiveParam.name) ?? null;
                                }
                                if (typeof tagParameter !== "undefined" && tagParameter !== null) {
                                    const tagReferenceResult = TagManager.instance?.resolveParameterCompletions(lastScopeItem.runtimeName(), tagParameter, params) ?? null;
                                ...
                                if (tActiveParam.hasInterpolations() && params.isInVariableInterpolation && lastScopeItem.isTagNode) {
                                    const interpolatedTagRef = TagManager.instance?.findTag(lastScopeItem.runtimeName());
            ...
            else if (params.isInVariableInterpolation &&
                params.currentNode == null) {
                completionItems = completionItems.concat(convertScopeToCompletionList(params, lastScopeItem.currentScope.ancestor()));
            }
            if (injectParentScope) {
                ...
                if (params.project.hasViewCollectionInjections(paramDocPath)) {
                    ...
                    ContentVariableNames.forEach((variableName) => {
                        completionItems.push({
                            label: variableName,
                            kind: CompletionItemKind.Field,
                        });
