...
import { ExcludeTaxonomyParams, SourceTaxonomyParams, } from "./resolveTaxonomyParameterCompletions.js";
export function getTaxonomyNames(symbol, statamicProject) {
    let taxonomyNames = [];
    ...
    if (typeof fromParam !== "undefined" && fromParam !== null) {
        if (fromParam.value.trim() === "*") {
            ...
            if (typeof restrictParam !== "undefined" && restrictParam !== null) {
                if (restrictParam.isVariableReference == false &&
            ...
            if (fromParam.isVariableReference == false &&
...
export function makeTaxonomyNameSuggestions(existingValues, project) {
    const items = [], taxonomyNames = project
