import { parseIdeHelper } from '../../idehelper/parser.js';
...
import { AntlersNode, EqualCompOperator, StringValueNode, VariableNode } from '../../runtime/nodes/abstractNode.js';
import TagManager from '../tagManagerInstance.js';
...
import { getSiteData, getSystemVariables } from '../variables/systemVariables.js';
import { checkNodeForPagination } from './factories/paginationFactory.js';
import { blueprintFieldsToScopeVariables, blueprintFieldToScopeVariable } from './scopeUtilities.js';
import { Scope } from './scope.js';
import FieldtypeManager from '../fieldtypes/fieldtypeManager.js';
import InjectionManager from './injections.js';
const IgnoreArrayContextualData = ['collection'];
const ChecksForFieldReferences = ['if', 'elseif', 'unless', 'elseunless'];
    ...
    analyzeScope(nodes) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            if (TagManager.instance?.isKnownTag(currentNode.getTagName()) == false) {
                ...
                if (i > 0) {
                    ...
                    if (lastSymbol.isComment && this.ideHelperMap.has(lastSymbol.id()) && !lastSymbol.isClosingTag && !currentNode.isClosingTag) {
                        const ideHelper = this.ideHelperMap.get(lastSymbol.id());
                ...
                if (currentNode.scopeVariable == null) {
                    ...
                    if (currentScope.hasList(trimmedRuntimeName) || currentNode.currentScope?.hasListInHistory(trimmedRuntimeName)) {
                        ...
                        if (listReference != null && listReference.values.size > 0) {
                            const firstListEntry = listReference.values.entries().next().value;
                            if (firstListEntry != null) {
                                const firstListVar = firstListEntry[1];
                    ...
                    if (speculativeBlueprintField != null) {
                        ...
                        if (currentNode.scopeVariable.sourceField != null) {
                            if (FieldtypeManager.instance?.hasFieldtype(currentNode.scopeVariable.sourceField.type)) {
                                const fieldTypeRef = FieldtypeManager.instance?.getFieldType(currentNode.scopeVariable.sourceField.type);
            ...
            if (TagManager.instance?.isKnownTag(currentNode.runtimeName())) {
                const tagRef = TagManager.instance?.findTag(currentNode.runtimeName());
